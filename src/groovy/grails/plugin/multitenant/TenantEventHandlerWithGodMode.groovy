...
class TenantEventHandlerWithGodMode extends TenantEventHandler{
	

	private Map<String, Class> reflectedCache = [:]
	
	public TenantEventHandlerWithGodMode() {
  	}
	...
	public boolean onPreInsert(PreInsertEvent preInsertEvent) {
		...
		if(hasAnnotation){
			...
			if(!setTenantId){
				//currentTenant.get() - returns actually tenant id number
				...
				int tenandIdIndex = findTenantIdIndex(properties)
				if (tenandIdIndex > -1) {
					preInsertEvent.getState()[tenandIdIndex] = [tenantToAdd];
				}
	...
	public void onLoad(LoadEvent event, LoadType loadType) {
		if (ConfigurationHolder.config.tenant.withGodMode && !currentTenant.isGodMode() && annotated(event.getEntityClassName())) {
   ...
   */
	...
	private attemptingTenantViolation(entityClassName, entity, currentTenantId){
		if (ConfigurationHolder.config.tenant.withGodMode && !currentTenant.isGodMode() && annotated(entityClassName)) {
			def loaded
